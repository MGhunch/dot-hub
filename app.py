...
def _parse_json_if_string(value):
    """
    PA sometimes sends arrays as JSON strings. Parse them back to arrays.
    e.g. "[\"file1.pdf\", \"file2.pdf\"]" -> ["file1.pdf", "file2.pdf"]
    """
...
def build_universal_payload(email_data, routing):
    ...
    return {
        ...
        # Attachments - handle PA sending arrays as JSON strings
        'hasAttachments': email_data.get('hasAttachments', False),
        'attachmentNames': _parse_json_if_string(email_data.get('attachmentNames', [])),
        'attachmentList': _parse_json_if_string(email_data.get('attachmentList', [])),
